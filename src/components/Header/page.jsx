...
export default function Header() {
    ...
    useEffect(() => {
        gsap.to(backgroundRef.current, { height: "100%", duration: 1, opacity: isActive ? 0.5 : 0, ease: "power4.inOut" });
        gsap.to(shopContainer.current, { opacity: isActive ? 0 : 1, duration: 0.5 });
    }, [isActive]);
            ...
            <div className={styles.bar}>
                ...
                <div onClick={() => { setIsActive(!isActive) }} className={styles.el}>
                    ...
                    <div className={styles.label}>
                        {!isActive && (
                            <p ref={openRef}>Menu</p>
                        )}
                        {isActive && (
                            <p ref={closeRef}>Close</p>
                        )}
                    </div>
                </div>
                <div ref={shopContainer} className={styles.shopContainer}>
                    <div className='hidden md:block'>
                        <UnderLinedText text="Shop" link="/shop" />
                    </div>
                    <div className={styles.el}>
                        ...
                        <UnderLinedText text="Cart(0)" link="/cart" />
                    </div>
                </div>
            </div>
